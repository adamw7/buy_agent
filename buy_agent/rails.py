...
to -- "the exact details of the Commerce Protocol are outside the scope of AP2"
-- so the counterparty is a choice, and a choice with one row per option is a
choice a third party can be added to without an ``if`` anywhere above this
module. A :class:`Rail` answers what the config could not: where it **listens**
(from its own environment variable), whether it needs an **address** and an
**enrolled key** at all, how a cart becomes a **merchant-signed checkout**, how
an authorisation is **presented**, which transport failures mean "not there", and
**how one is phrased**.

Two rails ship, and the default moves no money:

* ``dry-run`` plays every role there is. It signs the checkout itself, so the
  mandates are signed against a real hash and verify like any others, and then
  reports that nothing was charged. That is what makes the feature safe to turn
  on: ``--pay`` alone still spends nothing.
* ``http`` speaks to whatever AP2-speaking endpoint the operator names --
  ...
  to present the mandates. No merchant, wallet, processor or cloud is named here
  or anywhere else; the address is the whole of the integration.
...
behaviour. :class:`Cart` and :class:`~buy_agent.payment.Settlement` are what a
rail speaks in, so they live beside :mod:`buy_agent.payment`'s other domain types
rather than here -- the dependency runs one way, exactly as it does for a
provider.
"""
...
class Rail:
    ...
    Attributes:
        ...
        needs_endpoint: Whether an empty address is a usage error. False for the
            dry run, which has nowhere to be.
        needs_key: Whether signing requires a key somebody enrolled. False for the
            dry run, which has no counterparty to have trusted one -- see
            :func:`buy_agent.mandates.load_key`.
        moves_money: Whether a settlement here can actually charge anybody. Said
            on the row rather than worked out from the name, so both front doors
            can warn before a run rather than after one.
        ...
        transport_errors: The exceptions meaning the rail could not be reached, or
...
def _dry_run_checkout(cart: Cart, config: AgentConfig) -> tuple[SignedCheckout, str]:
    ...
    A key generated for this call and thrown away with it: the point of the dry
    run is that the mandate chain is real in shape -- a genuine hash of a genuine
    signed checkout, verifiable by the same code a merchant would run -- while
    being a credential nobody could redeem.
    """
    del config
...
def _post(url: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    A body that is not JSON is a :class:`~buy_agent.payment.PaymentError` and not
    a transport failure: something answered, so "could not reach it" would send
    the shopper to check a connection that is fine.
    """
...
def _http_checkout(cart: Cart, config: AgentConfig) -> tuple[SignedCheckout, str]:
    ...
    The hash is computed from what came back rather than taken from the answer:
    it is what the mandates bind to, and a hash the counterparty chose would let
    it bind them to a document nobody here has seen.
    """
...
def rail_options() -> list[dict[str, object]]:
    ...
    Carries each one's address and whether it needs one, so choosing a rail in
    the browser fills the field in and disables it where there is nothing to
    fill -- the same job ``provider_options`` does for a model server, and the
    same reason the page decides none of it itself.
    """
    return [
        {
            "name": rail.name,
