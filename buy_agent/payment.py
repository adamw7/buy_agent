"""Paying for a product the run found, once authorised (ADR-0009, ADR-0046, ADR-0006,
ADR-0017, ADR-0043)."""
...
class RailUnreachableError(PaymentError):
    """The counterparty was unreachable or its answer unreadable: its failure, so 502.
    A counterparty that declined stays a plain :class:`PaymentError`."""
...
class Cart(BaseModel):
    ...
    def label(self) -> str:
        """The price as every surface writes it."""
        return amount_label(self.price, self.currency)


class Receipt(BaseModel):
    """What came of a payment, as both front doors report it (ADR-0046)."""
    ...
    transaction_id: str
    reference: str
    #: Whether an open mandate, not a person, authorised this.
    autonomous: bool
    #: Whether the key was enrolled rather than generated by this process.
    enrolled_key: bool
    detail: str


def _check(product: Product, currency: str | None) -> tuple[float, str]:
    """The price and currency this product may be paid in, or a refusal (ADR-0043,
    ADR-0039)."""
    ...
    if placeable(currency) is None:
        # The vote may land on a spelling like "¥" (ADR-0054). Ranking scores that
        # neutral; paying refuses it.
        raise PaymentError(
...
def terms_for(
    ...
    """A cart's price and currency for this product, or why there is none (ADR-0033,
    ADR-0043)."""
    try:
...
def offer_for(product: Product) -> Offer | None:
    """The listing the headline price came from, matched on price and currency
    (ADR-0058, ADR-0022)."""
    return next(
        (
            offer
...
def _paid_to(product: Product) -> tuple[str, str]:
    """Who is paid and the page that quoted the price: off the headline's offer, else
    the product (ADR-0058)."""
...
def merchant_for(product: Product) -> str:
    """Who the cart would pay (ADR-0046, ADR-0058)."""
...
def cart_for(product: Product, products: Sequence[Product], config: AgentConfig) -> Cart:
    ...
    # Translated here so the form can mark ``products`` (ADR-0033).
    except ValueError as exc:
        raise PaymentError(str(exc), field="products") from exc
    # From the offer bought, not the product's link (ADR-0058).
...
def pay_for(cart: Cart, config: AgentConfig) -> Receipt:
    ...
    try:
        ...
        # Either rail call failing in transit is the same failure.
...
def _host(url: str | None) -> str:
    """The page's site, the fallback seller identity."""
