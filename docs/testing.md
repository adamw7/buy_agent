...
1053 Python tests and 123 UI tests. Nothing in either suite touches the network or
...
over ten fabricated pages `integration/conftest.py` owns -- a nightly failure
caused by DuckDuckGo rate-limiting or a shop redesigning its listing would report
nothing about this code. One session-scoped fixture runs the pipeline once, a CPU
