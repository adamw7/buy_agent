...
1119 tests run in about four seconds: most of that is the two that spawn an
...
reaching out. 1119 is what a machine with PowerShell collects *and* runs; with
neither `pwsh` nor `powershell` the same 1119 collect but 13 of the 17 in
`tests/test_start_script.py` skip, so the summary reads `1106 passed, 13 skipped`.
