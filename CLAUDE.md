...
1103 tests run in about four seconds: most of that is the two that spawn an
...
reaching out. 1103 is what a machine with PowerShell collects *and* runs; with
neither `pwsh` nor `powershell` the same 1103 collect but 13 of the 17 in
`tests/test_start_script.py` skip, so the summary reads `1090 passed, 13 skipped`.
